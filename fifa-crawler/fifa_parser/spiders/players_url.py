...
class SofifaSpider(scrapy.Spider):
	...
	def start_requests(self):
		urls = [
			'https://sofifa.com/players?col=oa&sort=desc&r={}&offset=0'.format(self.date) # FIFA ??
		]
	...
	def parse(self, response):
		...
		if offset != str(end_offset):
			next_href = '/players?col=oa&sort=desc&r={}&offset='.format(self.date) + str(int(offset)+60)
